...
class Session(pgraph.Graph):
    ...
    def ai_analyze_test_result(self, test_case_index, mutation_context, test_result):
        ...
        result_data = {
            ...
            'mutation_context': {
                'mutations': {k: {'index': v.index, 'name': v.name}
    ...
    def ai_adapt_strategy(self):
        ...
        self._fuzz_data_logger.log_info("🤖 AI正在分析测试结果并调整策略...")
        ...
        if effective_mutations:
            ...
            # 记录策略调整
            self._fuzz_data_logger.log_info(
                f"🎯 AI策略调整: 发现 {len(top_effective)} 个高效变异策略"
            )

            for mutation_name, effectiveness in top_effective:
                self._fuzz_data_logger.log_info(
                    f"   📈 {mutation_name}: {effectiveness:.2%} 有效率"
                )
            ...
            if qualified_name in self.ai_learning_data['effective_mutations']:
                ...
                if stats['total_tests'] > 20:
                    ...
                    if effectiveness < self.ai_decision_threshold:
                        self._fuzz_data_logger.log_info(
                            f"🤖 AI决策: 跳过低效变异 {qualified_name} (有效率: {effectiveness:.2%})"
                        )
                        return True
    ...
    def save_ai_learning_data(self, protocol: str = None):
        ...
        try:
            ...
            self._fuzz_data_logger.log_info(f"💾 AI学习数据已保存到: {ai_data_file}")
