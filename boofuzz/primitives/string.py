...
class String(Fuzzable):
    ...
    def __init__(
        ...
        self._string_cache = TieredCache(hot_size=500, warm_size=2000, cold_size=5000)
    ...
    def _generate_string_of_size(self, sequence: str, size: int) -> str:
        """Generate a string of specific size from sequence with advanced caching."""
        cache_key = (sequence, size)

        # Check tiered cache
        cached_result = self._string_cache.get(cache_key)
        ...
        if size <= 0:
            ...
            # More efficient string generation
            repeat_count = size // len(sequence)
            remainder = size % len(sequence)
            result = sequence * repeat_count + sequence[:remainder]

        # Store in tiered cache (automatically handles size limits)
        self._string_cache.put(cache_key, result)
    ...
    def _yield_long_strings(self, sequences):
        ...
        Optimized version with caching and efficient generation.

        @type  sequences: list(str)
        @param sequences: Sequence to repeat for creation of fuzz strings.
        """
        for sequence in sequences:
            # Pre-calculate size combinations to avoid repeated computation
            size_combinations = [
                length + delta
            ...
            for size in size_combinations:
                if self.max_len is None or size <= self.max_len:
                    data = self._generate_string_of_size(sequence, size)
                    yield data
                else:
                    break
            ...
            if self.max_len is not None:
                data = self._generate_string_of_size(sequence, self.max_len)
                yield data

        # Generate strings with null terminators at random positions
        for size in self._long_string_lengths:
            if self.max_len is None or size <= self.max_len:
                base_string = self._generate_string_of_size("D", size)
                for loc in self.random_indices[size]:
                    if loc < len(base_string):
                        yield base_string[:loc] + "\x00" + base_string[loc + 1 :]
            else:
                break
    ...
    def _get_mutation_hash(self, value: str) -> int:
        """Get a hash for mutation deduplication."""
        # Use hash of the first 1000 chars for very long strings to balance performance and accuracy
        if len(value) > 1000:
            return hash(value[:500] + str(len(value)) + value[-500:])
        return hash(value)

    def mutations(self, default_value):
        """
        Mutate the primitive by stepping through the fuzz library extended with the "this" library, return False on
        completion. Optimized version with advanced deduplication.
        ...
        Yields:
            str: Mutations
        """
        # Clear deduplicator for each new mutation cycle
        ...
        for val in itertools.chain(
            ...
            # Use advanced deduplication with Bloom filter
            if not self._deduplicator.is_duplicate(current_val):
                yield current_val
    ...
    def get_optimization_stats(self):
        ...
        cache_stats = self._string_cache.get_statistics()
        ...
        return {
            ...
            'optimization_summary': {
                ...
                'memory_usage_mb': (
                    cache_stats.get('hot_cache', {}).get('memory_usage_mb', 0) +
                    cache_stats.get('warm_cache', {}).get('memory_usage_mb', 0) +
                    cache_stats.get('cold_cache', {}).get('memory_usage_mb', 0)
                )
            }
        }
    ...
    def perform_maintenance(self):
        ...
        self._string_cache.maintenance()
