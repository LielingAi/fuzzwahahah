...
# Simple replacement for funcy.compose
...
class Bytes(Fuzzable):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._mutation_cache = TieredCache(hot_size=300, warm_size=1500, cold_size=3000)
        self._deduplicator = AdvancedDeduplicator(
            bloom_capacity=75000,
            bloom_error_rate=0.01,
            exact_cache_size=3000
        )
        ...
        with self._lock:
            ...
            for fuzz_value in self._iterate_fuzz_cases(default_value):
                ...
                if callable(fuzz_value):
                    cache_key = (id(fuzz_value), hash(str(default_value)))

                    # Check cache first
                    cached_result = self._mutation_cache.get(cache_key)
                    if cached_result is not None:
                        self._stats['cache_hits'] += 1
                        final_value = cached_result
                    else:
                        # Generate and cache
                        final_value = compose(self._adjust_mutation_for_size, fuzz_value)(default_value)
                        self._mutation_cache.put(cache_key, final_value)
                else:
                    cache_key = hash(fuzz_value)
                    cached_result = self._mutation_cache.get(cache_key)
                    if cached_result is not None:
                        self._stats['cache_hits'] += 1
                        final_value = cached_result
                    else:
                        final_value = self._adjust_mutation_for_size(fuzz_value=fuzz_value)
                        self._mutation_cache.put(cache_key, final_value)

                # Use advanced deduplication
                if not self._deduplicator.is_duplicate(final_value):
                    self._stats['total_mutations'] += 1
                    self._stats['unique_mutations'] += 1
                    self._stats['bytes_processed'] += len(final_value) if final_value else 0
                    yield final_value
                else:
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            cache_stats = self._mutation_cache.get_statistics()
            dedup_stats = self._deduplicator.get_statistics()

            processing_rate = (
                self._stats['bytes_processed'] / self._stats['generation_time']
            ...
            return {
                ...
                'performance': {
                    'bytes_per_second': processing_rate,
                    'cache_hit_rate': (
                        self._stats['cache_hits'] / self._stats['total_mutations'] * 100
                        if self._stats['total_mutations'] > 0 else 0
                    ),
    ...
    def perform_maintenance(self):
        ...
        with self._lock:
            self._mutation_cache.maintenance()
