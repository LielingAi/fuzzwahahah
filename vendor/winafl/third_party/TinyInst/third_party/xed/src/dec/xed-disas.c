...
// but never narrower than the actual encoded immediate width.
static unsigned int
xed_get_imm_sign_extend_display_bits(const xed_decoded_inst_t* p) {
    if ( xed3_operand_get_vexvalid(p) == 2 )
    {
        ...
        unsigned int imm_bits = xed_decoded_inst_get_immediate_width_bits(p);
        if (eff_bits > imm_bits)
            return eff_bits;
        if (imm_bits)
            return imm_bits;
    }
...
xml_print_flags(const xed_decoded_inst_t* xedd, char* buf, int blen)
{
    ...
    if (xed_decoded_inst_uses_rflags(xedd)) {
        ...
        if (xed_decoded_inst_has_default_flags_values(xedd)){
            ...
            if(okay) {
                blen = xml_tag(buf, blen, "of-default", dfv_mask.s.of);
                blen = xml_tag(buf, blen, "sf-default", dfv_mask.s.sf);
                blen = xml_tag(buf, blen, "zf-default", dfv_mask.s.zf);
                blen = xml_tag(buf, blen, "cf-default", dfv_mask.s.cf);
            }
        }
#endif
        for( i=0;i<nflags ;i++) {
...
/* Uses the XED autogenerated assembly conversion table, typically for handling operand decorations. */
...
xed_decoded_inst_dump_common_pre_mne(xed_print_info_t* pi)
{
    ...
    if (xed_operand_values_has_real_rep(ov)) {
        ...
        xed_prefixes(pi,"cs");  // non-JCC instruction with ignored CS prefix
    else if (pi->format_options.emit_ignored_branch_taken_hint
                 && xed_operand_values_ignored_branch_taken_hint(ov))
        xed_prefixes(pi,"ds");  // non-JCC instruction with ignored DS prefix
...
/* Emits disassembly components that appear after the mnemonic and before the operands.
 * (Current this is only the DFV) */
static void
xed_decoded_inst_dump_common_post_mne(xed_print_info_t* pi)
{
    ...
    if (xed_decoded_inst_uses_rflags(pi->p) && xed_decoded_inst_has_default_flags_values(pi->p)) {
        ...
        pi->blen = xed_print_cvt(pi->p, pi->buf, pi->blen, XED_OPERAND_CONVERT_DFVSTR);
    }
#endif
    (void)pi;
}
...
xed_decoded_inst_dump_intel_format_internal(xed_print_info_t* pi)
{
    ...
    xed_decoded_inst_dump_common_pre_mne(pi);
    ...
    xed_decoded_inst_dump_common_post_mne(pi);
