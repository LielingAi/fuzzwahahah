...
@click.option("--procmon-host", help="Process monitor port host or IP") # 调试器IP 
@click.option("--procmon-port", type=int, default=DEFAULT_PROCMON_PORT, help="Process monitor port") # 调试器端口
@click.option("--procmon-start", help="Process monitor start command") # 调试器启动命令
@click.option("--procmon-capture", is_flag=True, help="Capture stdout/stderr from target process upon failure") # 捕获目标进程的stdout/stderr
@click.option("--tui/--no-tui", help="Enable/disable TUI") # 是否启用TUI    
@click.option("--text-dump/--no-text-dump", help="Enable/disable full text dump of logs", default=False) # 是否启用完整文本日志
@click.option("--feature-check", is_flag=True, help="Run a feature check instead of a fuzz test", default=False) # 是否运行功能检查
@click.option("--target-cmd", help="Target command and arguments") # 目标命令和参数
@click.option(
