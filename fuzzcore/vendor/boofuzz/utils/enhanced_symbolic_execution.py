#!/usr/bin/env python3
"""
Heuristic Mutation & AI Learning Data Engine
启发式变异 + AI 学习数据引擎 - 协议数据生成、变异、学习反馈的统一接口

说明: 本模块不包含真正的符号执行 (SMT 求解), 命名保留历史兼容。
"符号执行" 的实际语义是: 基于协议模板词典的启发式变异 + boofuzz 原生变异。
"""
...
class ProtocolSymbolicEngine:
    """协议数据生成引擎 - 统一接口，集成AI学习功能 (启发式变异, 非SMT求解)"""
        ...
        try:
            ...
            for template_file in template_files:
                try:
                    ...
                    print(f"Warning: 协议模板文件格式错误 {template_file}: {e}")
                except Exception as e:
                    print(f"Error: 加载协议模板文件失败 {template_file}: {e}")

            if templates:
                print(f"成功加载 {len(templates)} 个协议模板")
            else:
                print("Warning: 未找到协议模板文件，使用默认模板")
                templates = self._get_default_protocol_templates()

        except Exception as e:
            print(f"Error: 加载协议模板时发生错误: {e}")
            print("使用默认协议模板")
            templates = self._get_default_protocol_templates()
    ...
    def _get_default_protocol_templates(self) -> Dict[str, Dict]:
        """获取默认协议模板（向后兼容 - 目录为空时使用内置最小数据）"""
        return {
            "echo": {
                "values": ["A", "HELLO", "test", "fuzz", "x" * 100, "%n%n%n", "\x00\xff"],
                "messages": ["Hello", "Test", "Echo", "PING", "x" * 256],
            },
            "generic": {
                "values": ["test", "admin", "A" * 1000, "' OR 1=1 --", "%s%s%s"],
            },
        }
    ...
    def generate_protocol_data(self, protocol: str, data_type: str, count: int = 10) -> List[Any]:
        ...
        try:
            ...
            # 如果变异失败，返回基础数据
            print(f"Warning: mutation generation failed for {protocol}.{data_type}: {e}")
    ...
    def generate_binary_data(self, data_type: str, length: int = 1) -> bytes:
        ...
        try:
            if data_type == 'random':
                ...
                import random
                # 内存上限保护: 单块最大 1MB, 避免 length*65535 产生内存炸弹
                max_length = min(length * 64, 1024 * 1024)
                length = random.randint(length, max_length)
                ...
                # 清理可能存在的同名请求，避免重复 s_initialize 抛异常
                if "overflow_data" in blocks.REQUESTS:
                    del blocks.REQUESTS["overflow_data"]
            ...
            print(f"Warning: boofuzz生成二进制数据失败: {e}")
            # 回退到简单的字节生成
    ...
    def convert_to_bytes(self, data: Any, format_type: str = 'auto') -> bytes:
        ...
        if isinstance(data, bytes):
            ...
            elif format_type == 'big_endian_word':
    ...
    def get_ai_enhanced_mutations(self, protocol: str, data_type: str, base_data: List, count: int) -> List[Any]:
        ...
        if mutation_key in self.mutation_success_rates:
            ...
            # 没有AI学习数据时，使用基础变异算法
            print(f"Info: 无AI学习数据，使用基础变异算法: {mutation_key}")
        ...
        try:
            ...
            for mutation_list in request.get_mutations():
                ...
                try:
                    ...
                    print(f"Warning: 处理变异数据失败: {e}")
                    continue
                
        except Exception as e:
            print(f"Warning: boofuzz生成失败，使用基础数据: {e}")
    ...
    def save_learning_data(self):
        ...
        try:
            ...
            print(f"Failed to save learning data file: {e}")
        except (TypeError, ValueError) as e:
            print(f"Learning data serialization failed: {e}")
        except Exception as e:
            print(f"Unknown error saving learning data: {e}")
    ...
    def load_learning_data(self):
        ...
        try:
            ...
            if os.path.exists(self.learning_data_file):
                ...
                print(f"Loaded AI learning data: {self.learning_data_file}")
        except (FileNotFoundError, PermissionError) as e:
            print(f"Cannot access learning data file: {e}")
            self.mutation_success_rates = {}
            self.crash_patterns = {}
        except (json.JSONDecodeError, KeyError) as e:
            print(f"Learning data file format error: {e}")
            self.mutation_success_rates = {}
            self.crash_patterns = {}
        except Exception as e:
            print(f"Unknown error loading learning data: {e}")
            self.mutation_success_rates = {}
            self.crash_patterns = {}
    ...
    def save_protocol_learning_data(self, protocol: str):
        ...
        try:
            ...
            print(f"Error: 保存{protocol}协议学习数据失败: {e}")
            return False
    ...
    def load_protocol_learning_data(self, protocol: str):
        ...
        try:
            ...
            if os.path.exists(protocol_file):
                ...
                print(f"Info: {protocol.upper()}协议学习数据文件不存在，将创建新的")
                return False

        except Exception as e:
            print(f"Error: 加载{protocol}协议学习数据失败: {e}")
            return False

# 全局实例 - 惰性初始化，避免 import 时产生副作用(建目录/打印/加载文件)
_protocol_symbolic_engine = None


def _get_engine() -> ProtocolSymbolicEngine:
    """惰性获取全局引擎实例"""
    global _protocol_symbolic_engine
    if _protocol_symbolic_engine is None:
        _protocol_symbolic_engine = ProtocolSymbolicEngine()
    return _protocol_symbolic_engine
...
def generate_protocol_data(protocol: str, data_type: str, count: int = 10, use_ai: bool = True) -> List[Any]:
    ...
    if use_ai:
        ...
        base_data = _get_engine().generate_protocol_data(protocol, data_type, max(1, count // 2))
        # 然后应用AI增强
        return _get_engine().get_ai_enhanced_mutations(protocol, data_type, base_data, count)
    else:
        return _get_engine().generate_protocol_data(protocol, data_type, count)


def generate_ai_enhanced_data(protocol: str, data_type: str, base_data: List, count: int) -> List[Any]:
    """便捷函数：获取AI增强的变异数据"""
    return _get_engine().get_ai_enhanced_mutations(protocol, data_type, base_data, count)
...
def generate_binary_data(data_type: str, length: int = 1) -> bytes:
    """便捷函数：生成二进制数据"""
    return _get_engine().generate_binary_data(data_type, length)

def convert_to_bytes(data: Any, format_type: str = 'auto') -> bytes:
    """便捷函数：转换为字节"""
    return _get_engine().convert_to_bytes(data, format_type)

def learn_from_test_result(protocol: str, data_type: str, mutation_data: str, test_result: Dict[str, Any]):
    """便捷函数：AI学习反馈"""
    _get_engine().learn_from_feedback(protocol, data_type, mutation_data, test_result)

def save_ai_learning_data(protocol: str = ""):
    """便捷函数：保存AI学习数据"""
    if protocol:
        _get_engine().save_protocol_learning_data(protocol)
    _get_engine().save_learning_data()

def load_protocol_ai_data(protocol: str):
    """便捷函数：加载协议特定的AI学习数据"""
    return _get_engine().load_protocol_learning_data(protocol)
