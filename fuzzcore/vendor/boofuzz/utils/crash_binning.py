...
# Simple replacement for envi.bits.hex function
def _hex_format(value, width=4):
    """Simple replacement for envi.bits.hex function."""
    try:
        if isinstance(value, int):
            return f"0x{value:0{width*2}x}"
        else:
            return f"0x{int(value):0{width*2}x}"
    except (ValueError, TypeError):
        return f"0x{str(value)}"
...
class CrashBinning:
  ...
  def dump_register_context(self, trace, regs):
    ...
    for i in regs.keys():
      if regs[i] != 0:
        ...
        register_string += f'{i}: \t{_hex_format(regs[i], 4)} \t{best}\n'
  ...
  def addr_to_module(self, trace, address):
      ...
      # Maybe its ASCII?
      try:
        hex_string = _hex_format(address, 4)[2:]
  ...
  def crash_synopsis (self, crash=None):
    ...
    synopsis =f'{crash.exception_module}: ' + \
              f'{_hex_format(crash.exception_address, 4)} ' + \
    ...
    if not crash.signal:
      ...
      synopsis += f'caused memory fault when attempting to ' + \
                  f'{direction} ' + \
                  f'{_hex_format(crash.violation_address, 4)}'
    else:
      if crash.signal == 11:
    ...
    if len(crash.disasm_around):
      ...
      for addr, op in crash.disasm_around:
        synopsis += f"\t{_hex_format(addr, 4)} {str(op)}\n"
    
    if len(crash.stack_unwind):
      synopsis += "\nCall stack:\n"
      for addr, frame, name in crash.stack_unwind:
        synopsis += f"\tAddress: {_hex_format(addr, 4)}\t Frame: {_hex_format(frame, 4)}\t name: {name}\n"

    if len(crash.seh_unwind):
      synopsis += "\nSEH chain:\n"
      for (addr, handler, name) in crash.seh_unwind:
        synopsis +=  f"\tAddress: {_hex_format(addr, 4)}\t Handler: {_hex_format(handler, 4)}\t name:{name}\n"
