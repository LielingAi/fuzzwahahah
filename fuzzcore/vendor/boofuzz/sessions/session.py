...
class Session(pgraph.Graph):
    ...
    def __init__(
        ...
        # AI自适应测试策略组件
        self.ai_strategy_enabled = True
        self.ai_learning_data = {
            'test_results': [],
            'crash_patterns': {},
            'effective_mutations': {},
            'target_response_patterns': {},
            'strategy_adjustments': []
        }
        self.ai_decision_threshold = 0.1  # AI决策阈值
        self.ai_adaptation_interval = 50   # 每50个测试用例进行一次策略调整
        self.ai_max_test_results = 1000    # 最大保存的测试结果数量，防止内存泄漏
    ...
    def _main_fuzz_loop(self, fuzz_case_iterator):
        ...
        try:
            ...
            for mutation_context in fuzz_case_iterator:
                ...
                # AI决策: 基于历史学习跳过低效变异 (total_mutant_index 已由迭代器自增)
                if self.ai_strategy_enabled and self.ai_should_skip_mutation(mutation_context):
                    self._fuzz_data_logger.log_info("AI decision: skipping low-efficiency mutation")
                    continue
            ...
            # 保存AI学习数据 (即使异常退出也不丢失本次运行的学习结果)
            try:
                self.save_ai_learning_data()
            except Exception:
                pass
    ...
    def _generate_mutations_for_request(self, path, skip_elements=None):
        ...
        mutations_iter = self.fuzz_node.get_mutations(skip_elements=skip_elements)

        # AI优先级重排: 历史学习到的高有效变异排到最前测试。
        # 只重排前 10000 个变异组合以控制内存, 其余保持原顺序。
        priority_names = set()
        if self.ai_strategy_enabled:
            try:
                priority_names = set(self.ai_get_priority_mutations())
            except Exception:
                priority_names = set()
        if priority_names:
            import itertools
            head = list(itertools.islice(mutations_iter, 10000))
            head.sort(key=lambda ms: 0 if any(
                m.qualified_name in priority_names for m in ms) else 1)
            mutations_iter = itertools.chain(head, mutations_iter)

        for mutations in mutations_iter:
            self.mutant_index += 1
            yield mutations
    ...
    def _fuzz_current_case(self, mutation_context):
        ...
        try:
            ...
            crashed = self._process_failures(target=target)
            # AI学习接线: 每个测试用例完成后分析结果, 积累有效变异统计
            if self.ai_strategy_enabled:
                try:
                    test_result = {
                        'crashed': crashed,
                    ...
                    self.ai_analyze_test_result(self.total_mutant_index, mutation_context, test_result)
                except Exception as e:
                    # AI学习失败不应中断模糊测试, 但记录便于排查
                    self._fuzz_data_logger.log_info(f"AI analysis failed: {e}")
            self._fuzz_data_logger.close_test_case()
            self.export_file()
    ...
    def ai_analyze_test_result(self, test_case_index, mutation_context, test_result):
        """AI分析测试结果并学习有效的变异策略"""
        if not self.ai_strategy_enabled:
            return
        ...
        result_data = {
            ...
            'mutation_context': {
                'mutations': {k: {'index': getattr(v, 'index', 0),
                                  'name': getattr(v, 'name', None) or getattr(v, 'qualified_name', str(v))}
                            for k, v in mutation_context.mutations.items()},
                'message_path': [e.dst for e in mutation_context.message_path]
            },
            'crashed': test_result.get('crashed', False),
            'response_time': test_result.get('response_time', 0),
            'response_size': test_result.get('response_size', 0),
            'error_occurred': test_result.get('error_occurred', False),
            'timestamp': time.time()
        }

        self.ai_learning_data['test_results'].append(result_data)

        # 内存管理：限制test_results的大小，防止内存泄漏
        if len(self.ai_learning_data['test_results']) > self.ai_max_test_results:
            # 保留最近的测试结果，删除最旧的
            self.ai_learning_data['test_results'] = self.ai_learning_data['test_results'][-self.ai_max_test_results:]

        # 分析崩溃模式
        if test_result.get('crashed', False):
            self._ai_analyze_crash_pattern(mutation_context, test_result)

        # 分析有效变异
        if (test_result.get('crashed', False) or
            test_result.get('response_time', 0) > 5.0 or
            test_result.get('response_size', 0) != test_result.get('expected_size', 0)):
            self._ai_record_effective_mutation(mutation_context, test_result)

        # 定期进行策略调整
        if test_case_index % self.ai_adaptation_interval == 0:
            self.ai_adapt_strategy()

    def _ai_analyze_crash_pattern(self, mutation_context, test_result):
        """分析崩溃模式"""
        crash_info = test_result.get('crash_info', {})
        crash_signature = f"{crash_info.get('signal', 'unknown')}_{crash_info.get('address', '0x0')}"

        if crash_signature not in self.ai_learning_data['crash_patterns']:
            self.ai_learning_data['crash_patterns'][crash_signature] = {
                'count': 0,
                'mutations': [],
                'common_features': {}
            }

        pattern_data = self.ai_learning_data['crash_patterns'][crash_signature]
        pattern_data['count'] += 1

        # 记录导致崩溃的变异
        for qualified_name, mutation in mutation_context.mutations.items():
            pattern_data['mutations'].append({
                'name': qualified_name,
                'index': mutation.index,
                'type': type(mutation).__name__
            })

    def _ai_record_effective_mutation(self, mutation_context, test_result):
        """记录有效的变异"""
        for qualified_name, mutation in mutation_context.mutations.items():
            if qualified_name not in self.ai_learning_data['effective_mutations']:
                self.ai_learning_data['effective_mutations'][qualified_name] = {
                    'total_tests': 0,
                    'effective_tests': 0,
                    'mutation_types': {}
                }

            stats = self.ai_learning_data['effective_mutations'][qualified_name]
            stats['total_tests'] += 1

            # 判断是否为有效测试
            if (test_result.get('crashed', False) or
                test_result.get('response_time', 0) > 5.0):
                stats['effective_tests'] += 1

            # 记录变异类型统计
            mutation_type = type(mutation).__name__
            if mutation_type not in stats['mutation_types']:
                stats['mutation_types'][mutation_type] = {'count': 0, 'effective': 0}

            stats['mutation_types'][mutation_type]['count'] += 1
            if (test_result.get('crashed', False) or
                test_result.get('response_time', 0) > 5.0):
                stats['mutation_types'][mutation_type]['effective'] += 1

    def ai_adapt_strategy(self):
        """AI自适应策略调整"""
        if not self.ai_strategy_enabled:
            return

        self._fuzz_data_logger.log_info("AI analyzing test results and adjusting strategy...")

        # 分析有效变异率
        effective_mutations = {}
        for qualified_name, stats in self.ai_learning_data['effective_mutations'].items():
            if stats['total_tests'] > 10:  # 有足够的数据
                effectiveness = stats['effective_tests'] / stats['total_tests']
                effective_mutations[qualified_name] = effectiveness

        # 识别最有效的变异策略
        if effective_mutations:
            sorted_mutations = sorted(effective_mutations.items(),
                                    key=lambda x: x[1], reverse=True)

            top_effective = sorted_mutations[:5]  # 前5个最有效的变异

            strategy_adjustment = {
                'timestamp': time.time(),
                'top_effective_mutations': top_effective,
                'crash_pattern_count': len(self.ai_learning_data['crash_patterns']),
                'total_tests_analyzed': len(self.ai_learning_data['test_results'])
            }

            self.ai_learning_data['strategy_adjustments'].append(strategy_adjustment)

            # 记录策略调整
            self._fuzz_data_logger.log_info(
                f"AI strategy adjustment: found {len(top_effective)} effective mutations"
            )

            for mutation_name, effectiveness in top_effective:
                self._fuzz_data_logger.log_info(
                    f"   {mutation_name}: {effectiveness:.2%} effectiveness"
                )

    def ai_should_skip_mutation(self, mutation_context):
        """AI决策：是否跳过当前变异"""
        if not self.ai_strategy_enabled:
            return False

        # 检查是否有足够的学习数据
        if len(self.ai_learning_data['test_results']) < 100:
            return False

        # 分析当前变异的历史表现
        for qualified_name, mutation in mutation_context.mutations.items():
            if qualified_name in self.ai_learning_data['effective_mutations']:
                stats = self.ai_learning_data['effective_mutations'][qualified_name]
                if stats['total_tests'] > 20:
                    effectiveness = stats['effective_tests'] / stats['total_tests']

                    # 如果有效率太低，考虑跳过
                    if effectiveness < self.ai_decision_threshold:
                        self._fuzz_data_logger.log_info(
                            f"AI decision: skip low-efficiency mutation {qualified_name} (effectiveness: {effectiveness:.2%})"
                        )
                        return True

        return False

    def ai_get_priority_mutations(self):
        """AI获取优先级变异列表"""
        if not self.ai_strategy_enabled:
            return []
        ...
        for qualified_name, stats in self.ai_learning_data['effective_mutations'].items():
            if stats['total_tests'] > 10:
                effectiveness = stats['effective_tests'] / stats['total_tests']
                if effectiveness > self.ai_decision_threshold * 2:  # 高于平均水平
                    priority_mutations.append((qualified_name, effectiveness))

        # 按有效性排序
        priority_mutations.sort(key=lambda x: x[1], reverse=True)

        return [name for name, _ in priority_mutations[:10]]  # 返回前10个

    def save_ai_learning_data(self, protocol: str = None):
        """保存AI学习数据到统一目录"""
        try:
            import json

            # 使用统一的AI学习数据目录
            ai_data_dir = "ai_learning_data"
            if not os.path.exists(ai_data_dir):
                os.makedirs(ai_data_dir)

            # 根据协议名称保存
            if protocol:
                ai_data_file = os.path.join(ai_data_dir, f"{protocol}_session_data_{self._run_id}.json")
            else:
                ai_data_file = os.path.join(ai_data_dir, f"session_data_{self._run_id}.json")

            # 简化数据以便JSON序列化
            simplified_data = {
                'protocol': protocol,
                'run_id': self._run_id,
                'crash_patterns': self.ai_learning_data['crash_patterns'],
                'effective_mutations': self.ai_learning_data['effective_mutations'],
                'strategy_adjustments': self.ai_learning_data['strategy_adjustments'],
                'total_tests': len(self.ai_learning_data['test_results']),
            ...
            self._fuzz_data_logger.log_info(f"AI learning data saved to: {ai_data_file}")

        except (PermissionError, OSError) as e:
            self._fuzz_data_logger.log_error(f"❌ 无法保存AI学习数据文件: {e}")
        except (TypeError, ValueError) as e:
            self._fuzz_data_logger.log_error(f"❌ AI学习数据序列化失败: {e}")
        except Exception as e:
            self._fuzz_data_logger.log_error(f"❌ 保存AI学习数据时发生未知错误: {e}")
