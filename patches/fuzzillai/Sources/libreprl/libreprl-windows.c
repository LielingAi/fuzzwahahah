...
static int reprl_spawn_child(struct reprl_context* ctx)
{
    ...
    // FuzzWahahah 修复: CreatePipe 成功返回非零, 原逻辑 ! 反转导致成功被当错误
    ...
    si.cb = sizeof(si);
    // FuzzWahahah 修复通道映射: JS 的 stdout/stderr 必须走 data channel
    // (child_stdout/child_stderr), 不能与 REPRL 控制通道混 — 否则 JS print/诊断
    // 字节进控制管道污染协议 (读 status 时先读到 "[COV" 等文本, signal='['=91)。
    // REPRL 控制写管道经环境变量 REPRL_CTRL_OUT_HANDLE 传给子进程 (引擎侧协议写专用)。
    si.hStdOutput = ctx->child_stdout ? ctx->child_stdout->hFile : INVALID_HANDLE_VALUE;
    ...
    // FuzzWahahah 修复: 原 strncpy_s(pointer, commandline - pointer + 1, ...) 的
    // 长度计算错误 (commandline - pointer 为负), 触发 strncpy_s 参数断言 abort。
    // 改用 memcpy + 手动偏移, 语义清晰且无参数断言风险。
    size_t length = 0;
        ...
        return reprl_error(ctx, "unable to allocate memory");
    {
        ...
        for (char **arg = ctx->argv; *arg; ++arg) {
            ...
            memcpy(commandline + off, *arg, len);
            off += len;
            commandline[off++] = ' ';
        }
    }
    ...
    //
    // Windows REPRL 引擎侧约定（FuzzWahahah 补丁）: 数据通道的 file mapping
    // HANDLE 通过环境变量传给子进程 (句柄继承保持值), 引擎侧 getenv 读取后
    // MapViewOfFile。控制通道固定用 stdin/stdout (对应 REPRL fd 100/101)。
    // 方向: ctx->data_out 是脚本通道 (fuzzer 写 → 引擎读);
    //       ctx->data_in  是覆盖回传通道 (引擎写 → fuzzer 读)。
    {
        char h_script[32], h_cov[32], h_ctrl_out[32];
        snprintf(h_script, sizeof(h_script), "REPRL_SCRIPT_HANDLE=%llu",
                 (unsigned long long)(uintptr_t)ctx->data_out->hFile);
        snprintf(h_cov, sizeof(h_cov), "REPRL_COV_HANDLE=%llu",
                 (unsigned long long)(uintptr_t)ctx->data_in->hFile);
        snprintf(h_ctrl_out, sizeof(h_ctrl_out), "REPRL_CTRL_OUT_HANDLE=%llu",
                 (unsigned long long)(uintptr_t)output[PI_WRITE]);
        _putenv(h_script);
        _putenv(h_cov);
        _putenv(h_ctrl_out);

        char **use_env = ctx->envp;
        char **merged_env = NULL;
        if (ctx->envp) {
            // 调用方给了自定义环境: 追加三个 HANDLE 变量
            size_t n = 0;
            while (ctx->envp[n]) n++;
            merged_env = calloc(n + 4, sizeof(char *));
            if (merged_env) {
                for (size_t i = 0; i < n; i++) merged_env[i] = ctx->envp[i];
                merged_env[n] = h_script;
                merged_env[n + 1] = h_cov;
                merged_env[n + 2] = h_ctrl_out;
                merged_env[n + 3] = NULL;
                use_env = merged_env;
            }
        }
        // ctx->envp 为 NULL 时 use_env 也是 NULL: 子进程继承当前进程环境
        // (含上面 _putenv 加入的 HANDLE 变量)
        //
        // FuzzWahahah 修复: CreateProcessA 的 lpEnvironment 是环境字符串块
        // ("K=V\0K=V\0\0"), 不是 POSIX 的 char** envp — 必须转换, 否则子进程
        // 环境损坏, SHM_ID/REPRL_SCRIPT_HANDLE 全部丢失。
        char *env_block = NULL;
        if (use_env) {
            size_t env_len = 1;  // 结尾额外 NUL
            for (char **e = use_env; *e; e++) env_len += strlen(*e) + 1;
            env_block = calloc(env_len, 1);
            if (env_block) {
                char *p = env_block;
                for (char **e = use_env; *e; e++) {
                    size_t l = strlen(*e);
                    memcpy(p, *e, l);
                    p += l + 1;
                }
        ...
        if (!CreateProcessA(NULL, commandline, NULL, NULL, TRUE, 0,
                            use_env ? env_block : NULL, NULL, &si, &pi))
...
int reprl_initialize_context(struct reprl_context* ctx, const char** argv, const char** envp, int capture_stdout, int capture_stderr)
{
    ...
    // FuzzWahahah 修复: hChild 必须初始化为 INVALID_HANDLE_VALUE(-1), 不能是
    // calloc 的 0 — reprl_execute 用 `hChild == INVALID_HANDLE_VALUE` 判断是否需要
    // spawn, 0 != -1 导致子进程永远不会被 spawn。
    ctx->hChild = INVALID_HANDLE_VALUE;
    return 0;
}
...
int reprl_execute(struct reprl_context* ctx, const char* script, uint64_t script_size, uint64_t timeout, uint64_t* execution_time, int fresh_instance)
{
    ...
    int status;
    DWORD dwBytesRead;
    // FuzzWahahah 修复: 原 WaitForSingleObject(hChild) 等子进程退出, 但 REPRL
    // 子进程是持久循环 (执行后回 status 不退出), WaitForSingleObject 永远超时。
    // 改为 PeekNamedPipe 轮询等 status 可读, 子进程退出则报错, 超时则 terminate。
    {
        DWORD waited = 0;
        const DWORD timeout_ms = timeout / 1000;
        int got = 0;
        while (waited < timeout_ms) {
            DWORD avail = 0;
            if (PeekNamedPipe(ctx->hControlRead, NULL, 0, NULL, &avail, NULL) && avail >= sizeof(status)) {
                got = 1;
                break;
            }
            DWORD ec;
            if (GetExitCodeProcess(ctx->hChild, &ec) && ec != STILL_ACTIVE) {
                reprl_child_terminated(ctx);
                return reprl_error(ctx, "child terminated during execution: %u", ec);
            }
            Sleep(1);
            waited++;
        }
