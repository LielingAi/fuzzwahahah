...
#if !os(Windows)
// PostgreSQL corpus/sync options. The PostgreSQL backend is not available on Windows,
// so these flags are ignored there (except --corpus=postgresql, which errors out below).
...
#if os(Windows)
    configError("The PostgreSQL corpus is not available on this platform (Windows)")
#else
...
#if os(Windows)
// The Windows CRT does not define SIGQUIT.
let terminationSignals = [SIGINT, SIGTERM]
#else
let terminationSignals = [SIGINT, SIGTERM, SIGQUIT]
#endif
